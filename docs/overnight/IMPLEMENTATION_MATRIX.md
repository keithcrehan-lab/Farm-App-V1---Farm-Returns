...
| 6 | Records | `PARTIAL` | `JobHistoryCard` (Vertical D, pre-existing) + `DecisionHistoryCard`/`listDecisionsForFarm` for decisions with no job attached, on a `/records` route. **Phase D (Evidence Ledger/provenance UX, 2026-09-03)** closed a real "why this number, later" gap: both cards now show the real evidence tier, calculation version, and inputs snapshot every decision has always carried — previously discarded once a decision was accepted/dismissed — 3 Codex audit rounds, `GATE: PASS` (0/0/0/0) at round 3, including a real fail-open fix (a dismissed decision's own row can carry an unvalidated `evidenceState`, now guarded against). `/reports` untouched, still has its own copy plus CSV/audit-trail tools. |
| 7 | Contextual Ask AI | `PARTIAL` | `AskAIButton`/overlay shipped, wired on Today/Plan/Records/Fields/Expanded Prompt/Confirm Actual/GPS Job Mode with real, inspectable context (never invented). **Phase C (contextual Ask AI completeness, 2026-09-03)** extended the context contract itself: `AskAIContext.facts` now carries real `EvidenceState`/Farmer-Actual provenance tags (reusing `src/domain/evidence.ts`'s own vocabulary) wherever a real tier exists, closing a genuine gap where a fact's real scientific tier (already shown to the farmer elsewhere on screen) was silently dropped before reaching Ask AI's own context — 3 Codex audit rounds, `GATE: PASS` (0/0/0/0) at round 3. No AI/LLM provider is configured in this repo (no API key, no server route) — the overlay states this plainly and fails closed rather than fabricating a response; wiring a real provider in later only needs to fill in one call, not rebuild the component. |
