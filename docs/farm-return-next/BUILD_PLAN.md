...
|---|---|---|
| A — Observe/telemetry | Phone GPS ingestion, IndexedDB offline outbox. Retention (~30-day raw, permanent derived evidence) and offline architecture (IndexedDB canonical, no service-worker-only queue, revision-based conflict detection) decided 2026-09-01 — see `ARCHITECTURE.md`. **First increment shipped 2026-09-01, 4 Codex audit rounds (1 Critical + 6 High + 2 Medium found and fixed, final round clean 0/0/0/0)**: `telemetry_events` migration + `telemetry_events_retention_job.sql` (real `pg_cron`, hourly) (both `PENDING_DEV_VALIDATION`) + `src/lib/farm-data/telemetry.ts` (idempotent, retry-safe insert) + `src/lib/offline/outbox.ts` (generic IndexedDB durable queue — farm-scoped, atomic claim + claimToken-guarded completion, explicit opt-in stale reclaim, real schema-upgrade path, all real per `fake-indexeddb` tests). Real GPS-capture wiring (`navigator.geolocation`) and any job-mode screen deliberately deferred to Vertical C — see `ARCHITECTURE.md`'s own scoping note. | Checkpoint 1's `observe/` |
...
| G — Notifications | In-app is the canonical first channel (product-owner decision, 2026-09-01) — real lifecycle states (unread/viewed/acted-on/dismissed/expired), contextual/actionable content, built independent of any push vendor. Push is a future adapter, not a blocker. **First increment shipped 2026-09-01**: `notifications` migration (schema, trigger-enforced state machine, real hourly `pg_cron` expiry job, `PENDING_DEV_VALIDATION`) + `src/lib/farm-data/notifications.ts` + new `src/orchestration/notify/` stage (`notificationFromPrompt`, copies title/body verbatim from a real OK-status Prompt, never invents copy). No notification-centre UI yet — same "no screen ahead of an approved reference" scoping as Vertical A's own first increment; Today's own visual reference is also not yet approved (`UX_DESIGN.md`). | Vertical B |
| H — Satellite field intelligence | Copernicus Data Space Ecosystem, Sentinel-2 L2A surface reflectance, behind a provider boundary (product-owner decision, 2026-09-01) — see `BLOCKERS.md` for the full evidence/provenance requirements. Field/vegetation intelligence only this phase; NDVI is never presented as direct biomass. **First slice shipped 2026-09-01**: `src/server/satellite/cdse-stac-client.ts` (real, live-verified, unauthenticated Sentinel-2 L2A STAC search — no migration, no credentials needed for catalogue search) + `src/domain/satellite-field-coverage.ts` (picks the best real scene for a field by real cloud cover + real footprint intersection). Real NDVI/vegetation-index computation from raw bands needs CDSE `oidc`/`s3` credentials this session cannot obtain (account creation prohibited) — see `BLOCKERS.md`. | none |
