...
| `LabResult` | **Built, Checkpoint 2.** A new `lab_results` row, `job_session_id` unique — references `job_sessions.id` (the CompositeSample) directly, never a `soil_core_observations` row. Raw measured values only (pH, Morgan's P/K mg/L, Mg, OM%, lab-reported lime requirement) — insert-only, immutable. | Structurally enforces "one composite sample -> one lab result" — the unique constraint, not just application code. |
| `SoilInterpretation` | **Built, Checkpoint 2.** A new `soil_interpretations` row, `lab_result_id` — insert-only, versioned (a methodology change inserts a new row; the most recent is "current"). Derived via `src/domain/soil-interpretation.ts`'s `interpretLabResult`, which reuses (never re-derives) `nutrients.ts`'s own `pIndexFromMgL`/`kIndexFromMgL`. | Then applied to the field's real `fertility` via the *existing, unmodified* `addSoilTestToField` (`src/lib/farm-data/soil.ts`) — the same function the legacy manual Soil-screen entry already calls — so Checkpoint 3's already-built `calculateNutrientPlan` picks it up with zero engine changes. `SoilTest` gained additive `compositeSampleId`/`labResultId` provenance fields. |
