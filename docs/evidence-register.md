...
| Teagasc — Producing beef from grass-forage-based systems (1 Jul 2026, Spring 2026 Grange Feed Costings Model) | **Real data in hand, not yet implemented.** `farm_return_core_data_v4.xlsx`'s `Feed_Cost_2026` sheet: €/t DM grown/utilised with and without land charge, energy values, DM yields. Closes the silage/grass cost-driver gap `FeedCostOverviewCard` still shows as mock — next candidate for a finance-engine pass. | https://teagasc.ie/insights/producing-beef-from-grass-forage-based-systems/ |
| Teagasc — Nutritional management of finishing beef cattle (1 Jul 2026) + Response in Beef Cattle to Concentrate Feeding in Winter (Mar 2001, evidence class B) | **Real data in hand, not yet implemented.** `Steer_2026_Budget`/`Steer_Trial_Evidence`/`Steer_3_Strategy` sheets: a current continental steer budget and flat/step/ad-lib trial evidence — the same *shape* of data (concentrate level -> observed response) that made the Weanling three-strategy optimiser real. Would close the "Continental Steers strategy comparison stays Phase 1 mock" gap in `docs/feed-engine.md`. | https://teagasc.ie/insights/nutritional-management-of-finishing-beef-cattle/ |
| Met Éireann — Climate Services (SMD model) | **Real specification in hand, not yet implemented.** `MetEireann_SMD` sheet: drainage-class SMD/PE/AE structures and an app-rule schema for the Phase 5 weather/spreading engine — this is model specification, not live daily weather data, so ingestion still needs either API access or periodic data supply. | https://www.met.ie/climate/services |
| — Bulk-buy supplier quote schema | **Illustrative schema only, not real data** — `Bulk_Buy_Schema`'s three example rows are explicitly marked "Example only" in the sheet itself. Defines the shape a real Input Planner bulk-buying feature would need (supplier, product, quantity, all-in price, benchmark, saving), but no compliance/finance logic should be built from these numbers — a live commercial quote is required per the sheet's own note. | (workbook only, no public URL) |
