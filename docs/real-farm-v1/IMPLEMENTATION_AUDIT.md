...
| `/spreading` | Real Met Éireann observations/forecast, real `spreading-legal-gate.ts`/closed-period calendar (per `git log` "Closure pass Priority 8: /spreading live-wires the real closed-period calendar") | — | Per README's own Phase 5 note, the old unsourced 0–100 spreading score does **not** appear to still be live (worth a direct re-check against the brief's explicit "do not resurrect" instruction before declaring this closed). |
