...
import InputPlannerPage from "./page";
...
// entirely — a real excluded field left the Fertiliser breakdown/Forecast
// Spend total looking complete when it understated the truth.
describe("InputPlannerPage", () => {
  it("discloses a blocked field excluded from the Fertiliser forecast", () => {
    renderPage([field()], []);
    expect(screen.getByText(/1 field excluded from the/i)).toBeTruthy();
  ...
  it("never shows the exclusion disclosure when every real field's evidence is complete", () => {
    ...
    expect(screen.queryByText(/excluded from the/i)).toBeNull();
  });
});
